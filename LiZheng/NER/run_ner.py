...
def evaluate(args, model, device, dev_labels, dev_dataloader):
        ...
        with torch.no_grad():
            if args.with_crf:
                ...
                all_predictions = torch.cat(
                    (all_predictions, predicted_lbl), dim=0)
    ...
    dev_labels.sub_(torch.ones(dev_labels.size(
...
def main(args):
    ...
    data_processor = DataProcessor(
        "./data/conll04_train.json", "./data/conll04_dev.json", "./data/conll04_test.json")
    (train_examples, dev_examples), vocabulary = data_processor.get_conll_examples(
        do_training=True)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_batches):
            ...
            if (step + 1) % eval_step == 0:
                ...
                save_model = False
                if args.do_eval:
                    score = evaluate(args, model, device,
                                     dev_label, dev_dataloader)
                ...
                if save_model:
                    ...
                    if max_score:
                        with open(os.path.join(args.output_dir, "eval_results.txt"), "w") as writer:
                            writer.write(
                                "Best eval result: F1 = %.4f" % max_score)
    if args.do_eval:
        [test_examples], _ = data_processor.get_conll_examples(
            do_training=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--do_eval", action="store_true",
                        help="Do eval and test.")
    parser.add_argument("--with_crf", action="store_true",
                        help="Use BiLSTM_CRF model.")
    parser.add_argument("--batch_size", default=16, type=int,
                        help="Batch size for training.")
    parser.add_argument("--hidden_size", default=128,
                        type=int, help="Hidden size for lstm.")
    parser.add_argument("--num_of_tags", default=7, type=int,
                        help="Number of meaningful tags.")
    parser.add_argument(
