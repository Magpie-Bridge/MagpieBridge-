...
class DataProcessor:
    def __init__(self, train_file, dev_file, test_file):
        ...
        self.type_to_label = {"Peop": "PER",
                              "Org": "ORG", "Loc": "LOC", "Other": "O"}
        self.label_to_id = {
            "<PAD>": 0,
            "B-PER": 1,
            "I-PER": 2,
        ...
        for i in range(file_num):
            ...
            for data in dataset:
                ...
                label = [self.type_to_label["Other"]] * token_len + \
                    ["<PAD>"] * (self.max_seq_len - token_len)
                for entity in entities:
                    ...
                    for idx in range(entity["start"], entity["end"]):
                        if idx == entity["start"]:
                            label[idx] = "B-" + \
                                self.type_to_label[entity["type"]]
                        else:
                            label[idx] = "I-" + \
                                self.type_to_label[entity["type"]]
    ...
    def convert_example_to_features(self, examples, vocab):
        examples = sorted(
            examples, key=lambda x: true_len(x.tokens), reverse=True)
        features = []
        labels = []
